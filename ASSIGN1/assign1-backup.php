...
if($_POST){
	...
	if(!preg_match("/^[0-9]+(\.[0-9][0-9]?)?$/" , $_POST['pricemax'])){
			$valid = false;
			$maxerr = "must be a decimal point number";
	}
	...
	if(!preg_match("/^[0-9]+(\.[0-9][0-9]?)?$/", $_POST['pricemin'])){
			$valid = false;
			$minerr = "must be a decimal point number";		
	}
...
if($_POST && $valid){
	...
	$date = mysqli_query($cxn, $query2) or die ('ERROR QUERY' . mysqli_error($cxn));
